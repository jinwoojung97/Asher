...
final class TalkView: UIView {
  ...
  func scrollToBottom() {
    let indexPath = IndexPath(row: DataSource.messages.count - 1, section: 0)
  ...
  private func bind() {
    chatInputView.viewModel.output?.updateTableView
      .drive { [weak self] _ in self?.updateTableView() }
  ...
  private func getTableViewCell(
    ...
    guard let cell = tableView.dequeueReusableCell(
      ...
      for: indexPath
          ...
          let message = DataSource.messages[safe: indexPath.row]
